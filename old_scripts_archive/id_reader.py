...
import os, sys, argparse, glob, tempfile, time, random, re, json
...
# -------- Namespaces (tight regexes; no capture groups needed) --------
NS_PAT = {
    # GEO / NCBI
    ...
    # Generic sample-like keys often seen in pipelines
    "UUID":      re.compile(r"\b[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}\b", re.I),
}

# Breast/nano quick tags (header-only; cheap)
...
ID_CAND_COLS = [  # broad, but still specific column hints to scan first
...
def find_header_ns(cols):
    ns_hits={}
    for c in cols:
        s=str(c)
        for ns,pat in NS_PAT.items():
            if pat.search(s):
                ns_hits.setdefault(ns,set()).add(s)
    return {k:sorted(v) for k,v in ns_hits.items()}
...
def scan_values(series):
    ...
    for v in series.fillna("").astype(str).tolist():
        for ns,pat in NS_PAT.items():
            if pat.search(v):
                counts[ns]+=1
                if len(examples[ns])<5: examples[ns].append(v)
...
def analyze_file(path):
    ...
    try:
        if ext in (".csv",".tsv",".txt"):
            ...
            # 1) header namespaces (rare, but record)
            hns=find_header_ns(cols)
            # 2) wide header guess (expression/assays where columns are sample IDs)
            non_gene=[c for c in cols if c.lower() not in GENE_COLS]
            wide_candidates=[c for c in non_gene if any(p.search(str(c)) for p in NS_PAT.values())]
            # 3) targeted value scan in likely ID columns, else shallow scan across all (bounded)
            first_cols=[c for c in cols if c.lower() in ID_CAND_COLS]
            pick = first_cols or cols[: min(12, len(cols))]
            ...
            if not df.empty:
                for c in pick:
                    ...
                    for ns,vals in ex.items():
                        vlist=v_examples.setdefault(ns,[])
                        for x in vals:
                            if len(vlist)<5 and x not in vlist: vlist.append(x)
            # Decide mode & finalize
            ns_union=set(hns.keys()) | set(v_counts.keys())
            row["mode"] = "wide_header" if len(wide_candidates)>=3 else ("value_scan" if v_counts else ("header_only" if hns else "none"))
            row["id_columns"] = ";".join(first_cols or [])
            ...
            if not cols: return row
            row["header_tags"]=header_tags(cols)
            # wide header detection
            non_gene=[c for c in cols if str(c).lower() not in GENE_COLS]
            wide_candidates=[c for c in non_gene if any(p.search(str(c)) for p in NS_PAT.values())]
            # value scan on likely columns only (avoid heavy full scans)
            first_cols=[c for c in cols if str(c).lower() in ID_CAND_COLS]
            v_counts={}; v_examples={}
            for c in first_cols[:12]:
                vals=pq_col(path, c)
            ...
            row["mode"] = "wide_header" if len(wide_candidates)>=3 else ("value_scan" if v_counts else "none")
            row["id_columns"] = ";".join(first_cols or [])
            row["namespaces"] = ";".join(sorted(ns_union))
            row["ns_counts"] = ";".join(f"{k}={v_counts.get(k,0)}" for k in sorted(ns_union)) if v_counts else ""
            row["examples"]  = ";".join(f"{k}:" + "|".join(v_examples.get(k,[])) for k in sorted(v_examples)) if v_examples else ""
...
def main():
    ...
    if RICH:
        with Progress(TextColumn("[bold]scan"), BarColumn(), TextColumn("{task.completed}/{task.total}"), TimeRemainingColumn(), console=console, transient=True) as prog:
            ...
            # batch per worker to keep bar accurate
