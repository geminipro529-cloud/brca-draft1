#!/usr/bin/env python3
# Stage 2: Merge RNA-seq expression with metadata (Parquet from data/01_raw/RNASEQ/<source>)
...
def pick_expression_file(source: str, prefer_parquet: bool) -> Path:
    ...
    if not rnaseq_dir.exists():
        sys.exit(f"[ERROR] RNASEQ folder not found: {rnaseq_dir}")
    ...
    sys.exit(f"[ERROR] No RNA-seq files found in {rnaseq_dir}")

def load_expression(path: Path, preview_rows=50):
...
def merge_expression(expr: pd.DataFrame, meta: pd.DataFrame, outdir: Path, source: str):
    ...
    if not key:
        sys.exit(f"[ERROR] No common key found. Expression cols: {expr.columns[:10]} | Metadata cols: {meta.columns[:10]}")
...
def main():
    ...
    ap.add_argument("--metadata", default="data/02_interim")
    ap.add_argument("--out", default="data/03_processed")
    ap.add_argument("--prefer-parquet", action="store_true")
    args = ap.parse_args()
...
if not expr_path.exists():
    sys.exit(f"[ERROR] Expression not found: {expr_path}")

meta = load_metadata(meta_path)
expr, preview_expr = load_expression(expr_path, preview_rows=50)

print(f"[DEBUG] Metadata cols: {list(meta.columns)[:20]}")
print(f"[DEBUG] Expression cols: {list(expr.columns)[:20]}")
