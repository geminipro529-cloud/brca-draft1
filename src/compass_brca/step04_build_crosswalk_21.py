...
# DEFINITIVE VERSION
# This version is fully integrated with the global pipeline configuration.
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOGS_DIR / "step04_build_crosswalk.log")],
)
...
def process_single_file(file_path_str: str) -> pl.DataFrame:
    ...
    try:
        ...
        for i in range(0, len(value_vars), STEP04_COLUMN_CHUNK_SIZE):
            chunk_value_vars = value_vars[i:i + STEP04_COLUMN_CHUNK_SIZE]
        ...
        logger.error(f"Failed to process file {file_path_str}: {e}")
        return pl.DataFrame(schema=schema)

def main():
    """Main orchestrator for building the crosswalk using Dask."""
    console.print(Panel(
        f"[bold cyan]Step 04: ID Crosswalk Builder[/]\n"
        f"[dim]Engine: Dask | Workers: {DASK_N_WORKERS} | Batch Size: {STEP04_BATCH_SIZE}[/]",
        expand=False
    ))
    ...
    if not SMALL_FILES_LIST.exists() or not LARGE_FILES_LIST.exists():
        ...
        sys.exit(1)
    logger.info("Stratified file lists found. Proceeding.")
    ...
    with LocalCluster(n_workers=DASK_N_WORKERS, memory_limit=DASK_MEMORY_LIMIT, threads_per_worker=1) as cluster, Client(cluster) as client:
        ...
        with Progress(TextColumn("[progress.description]{task.description}"), BarColumn(), TextColumn("[magenta]{task.completed}/{task.total} files"), TimeElapsedColumn()) as progress:
            ...
            for i in range(0, len(all_files), STEP04_BATCH_SIZE):
                batch_files = all_files[i:i + STEP04_BATCH_SIZE]
    ...
    if not all_results:
        logger.error("No valid data was processed. Crosswalk will be empty.")
